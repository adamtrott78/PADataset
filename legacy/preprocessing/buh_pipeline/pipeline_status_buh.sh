...
check_one() {
  ...
  for sid in $shards; do
    ...
    # decide NEXT
    local next="DONE"
    if [[ "$mode" == "pa1_merge" && $bank_ok -eq 1 ]]; then
      next="READY_MERGE"
...
check_one "wifi"      "$CORE_SUFFIX" "$CORE_BANK" "$CORE_NOISE" "normal" "$CORE_SHARDS" "${CORE_PAS[@]}"
check_one "bluetooth" "$CORE_SUFFIX" "$CORE_BANK" "$CORE_NOISE" "normal" "$CORE_SHARDS" "${CORE_PAS[@]}"
check_one "zigbee"    "$CORE_SUFFIX" "$CORE_BANK" "$CORE_NOISE" "normal" "$CORE_SHARDS" "${CORE_PAS[@]}"

check_one "wifi"      "$PA1_SUFFIX" "$PA1_BANK" "$PA1_NOISE" "pa1_merge" "$PA1_SHARDS" "${PA1_PAS[@]}"
check_one "bluetooth" "$PA1_SUFFIX" "$PA1_BANK" "$PA1_NOISE" "pa1_merge" "$PA1_SHARDS" "${PA1_PAS[@]}"
check_one "zigbee"    "$PA1_SUFFIX" "$PA1_BANK" "$PA1_NOISE" "pa1_merge" "$PA1_SHARDS" "${PA1_PAS[@]}"
