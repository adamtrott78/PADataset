function [Xsig, sched] = pa_gen_windows_pa8_stream(cfg, wlanCfg, session_id, tape_id, plan, window_ids)
%PA_GEN_WINDOWS_PA8_STREAM Signal-only PA8 windows (each window contains repeats by design).
% Determinism rules:
%   - window_ids is REQUIRED and is the sole per-window identity for seeding.
% Evidence span for PA8 is TRAIN SPAN (first start -> last end), not duty.
% Generator enforces train-span >= evidence_span_frac_min by construction.
    ...
    if nargin < 6
        error("pa_gen_windows_pa8_stream requires window_ids (length M).");
    end
    ...
    % IMPORTANT: PA8 is window-intrinsic; we use a fixed segment_id_seed=0 for pa_sha_seed signature.
    segment_id_seed = 0;
    ...
    % PA8 params
    rc_rng   = double(pa_get_nested(cfg,"pas.PA8.params.repeat_count"));      % [2,4]
    sp_rng_s = double(pa_get_nested(cfg,"pas.PA8.params.repeat_spacing_s"));  % [0.003,0.007]
    tl_rng_s = double(pa_get_nested(cfg,"pas.PA8.params.template_len_s"));    % [0.0005,0.002]
    ...
    for i = 1:M
        window_id = double(window_ids(i)); % REQUIRED identity
        ...
        % choose repeat_count, but enforce >=3 so span>=0.75 is achievable under spacing bounds
        ...
        tlen_req = span_target - (repeat_count-1)*spacing_max;   % minimum tlen if spacing=spacing_max
    ...
    gap_min = max(1, round(0.0002 * Fs)); % 0.2 ms guard so components stay separated
    
    % base requirement from span target
    spacing_req = ceil((span_target - tlen) / max(1, repeat_count-1));
    
    % enforce non-overlap: start-to-start spacing must exceed template length + gap
    ...
    spacing = min(max(spacing_req, spacing_min), spacing_max);
        ...
        tpl = make_template(cfg, master_seed, schema, session_id, tape_id, segment_id_seed, window_id, wlanCfg, tlen, rs);

        % place repeats into a W-length window
        [xw, near_params] = place_repeats(W, tpl, mode, near, master_seed, schema, session_id, tape_id, segment_id_seed, window_id, Fs, intervals);

        % --- peak-after-normalization safety (deterministic) ---
        rms0 = pa_rms(xw); pk0 = max(abs(xw));
        if rms0 > 0 && (double(pk0) / double(rms0)) > peak_thr
            ...
            tpl2 = make_template(cfg, master_seed, schema, session_id, tape_id, segment_id_seed, window_id+7777, wlanCfg, tlen2, rs);
            [xw, near_params] = place_repeats(W, tpl2, mode, near, master_seed, schema, session_id, tape_id, segment_id_seed, window_id, Fs, intervals2);

            repeat_count = repeat_count2; tlen = tlen2; spacing = spacing2; t0 = t02; intervals = intervals2;
        end
        ...
        sched(i).repeat_spacing_samp = spacing;
        sched(i).t0_samp = t0;
...
function tpl = make_template(cfg, master_seed, schema, session_id, tape_id, segment_id, window_id, wlanCfg, tlen, rs)
%MAKE_TEMPLATE Build a deterministic replay template snippet (cropped IQ),
%optionally with "recorded noise" baked in under a config flag.
    ...
    while numel(buf) < need
        [~, seed_payload] = pa_sha_seed(master_seed, schema, session_id, tape_id, segment_id, window_id*100000 + double(pkt_idx), "payload");
        [~, seed_scr]     = pa_sha_seed(master_seed, schema, session_id, tape_id, segment_id, window_id*100000 + double(pkt_idx), "scrambler");
...
function [xw, sim_params] = place_repeats(W, tpl, mode, near, master_seed, schema, session_id, tape_id, segment_id, window_id, Fs, intervals)
    ...
    for r = 1:repeat_count
        a = intervals(r,1); b = intervals(r,2);
        if a < 1 || b > W || b < a, continue; end
