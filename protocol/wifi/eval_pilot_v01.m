 function eval_pilot_v01()
    P = pa_paths();
%EVAL_PILOT_V01 Evaluate saved pilot windows and emit evidence pack.

    cfg = pa_load_cfg(fullfile(P.config,"starter.json"));
    pa_validate_cfg(cfg);

    out_root  = P.results_wifi_digital_eval_pilot;
    data_root = P.data_wifi_pilot;
    ev_root   = P.results_wifi_digital_eval_pilot_evidence;
    ...
    typical = struct();   % typical.(PA) = struct(idx,x,vr)
    special = struct();   % special.PA4_as_PA3, special.PA4_rejected
    ...
    for pi = 1:K
        ...
        for i = 1:N
            ...
            % --- cross validation (PA2/PA3/PA4 only) ---
            for ej = 1:Ke
                pa_eval = evalPAs(ej);
    ...
    save(fullfile(out_root,"pilot_summary_v01.mat"), "cfg","T","accept_counts","total_counts","PAs","evalPAs","typical","special");
    fprintf("Saved: %s\n", fullfile(out_root,"pilot_summary_v01.mat"));

    % ---- write evidence pack figures ----
...
function write_typical(ev_root, cfg, typical)
    ...
    for pa = PAs
        ...
        pilot_plot_evidence(pa, cfg, ex.x, ex.vr, out, []);
    end
end
...
function write_special(ev_root, cfg, special)
    ...
    if isfield(special,"PA4_as_PA3")
        ...
        pilot_plot_evidence("PA4", cfg, ex.x, ex.vr_pa4, out, ex.vr_pa3);
    end
    if isfield(special,"PA4_rejected")
        ex = special.PA4_rejected;
        out = fullfile(d, sprintf("PA4_rejected_idx%03d.png", ex.idx));
        pilot_plot_evidence("PA4", cfg, ex.x, ex.vr, out, []);
    end
end

function pilot_plot_evidence(pa, cfg, x, vr, out_png, vr_alt)
% 3-panel figure:
%  (1) envelope + evidence mask overlay (+ repeat intervals for PA8)
%  (2) spectrogram
%  (3) centroid-bin trace (and jump count)
    ...
    if string(pa)=="PA2"
        ...
        e23 = pa_detect_E2E3(x, round(Fs), double(bp.power_smoothing_s), double(bp.refractory_s), double(bp.thresholds.hi_mad_k), double(bp.thresholds.lo_mad_k));
    ...
    elseif string(pa)=="PA8"
        if isfield(vr,"det") && isfield(vr.det,"repeat") && isfield(vr.det.repeat,"intervals") && ~isempty(vr.det.repeat.intervals)
            iv = vr.det.repeat.intervals;  % [R x 2] from signal-derived detector
            a = iv(1,1); b = iv(end,2);
    ...
    if string(pa)=="PA8"
        if isfield(vr,"det") && isfield(vr.det,"repeat") && isfield(vr.det.repeat,"intervals") && ~isempty(vr.det.repeat.intervals)
            ...
            for r = 1:size(iv,1)
                a = iv(r,1); b = iv(r,2);
                xline(tms(a), "b-"); xline(tms(b), "b-");
            end
        end
    end
    grid on; xlabel("Time (ms)"); ylabel("|x|^2");
    ttl = sprintf("%s | valid=%d | %s", pa, vr.valid, vr.reject_reason);
    if ~isempty(vr_alt)
