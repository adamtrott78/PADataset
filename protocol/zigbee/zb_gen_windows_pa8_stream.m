function [Xsig, sched] = zb_gen_windows_pa8_stream(cfg, session_id, tape_id, plan, window_ids)
%ZB_GEN_WINDOWS_PA8_STREAM Signal-only PA8 windows for Zigbee.
% Mirrors Wi-Fi/Bluetooth PA8 semantics:
%   - window_ids REQUIRED
%   - repeat_spacing_s is START-TO-START spacing
%   - repeat_count forced to >= 3
%   - evidence span enforced by construction
    ...
    if nargin < 5
        error("zb_gen_windows_pa8_stream requires window_ids (length M).");
    end
    ...
    sp_rng_s = double(pa_get_nested(cfg,"pas.PA8.params.repeat_spacing_s"));
    ...
    for i = 1:M
        ...
        tpl = make_template(cfg, zbCfg, master_seed, schema, session_id, tape_id, segment_id_seed, window_id, tlen, rs);

        [xw, near_params] = place_repeats(W, tpl, mode, near, master_seed, schema, ...
            session_id, tape_id, segment_id_seed, window_id, Fs, intervals);
        ...
        if rms0 > 0 && (double(pk0) / double(rms0)) > peak_thr
            ...
            tpl2 = make_template(cfg, zbCfg, master_seed, schema, session_id, tape_id, segment_id_seed, window_id+7777, tlen2, rs);
            [xw, near_params] = place_repeats(W, tpl2, mode, near, master_seed, schema, ...
                session_id, tape_id, segment_id_seed, window_id, Fs, intervals2);
...
function tpl = make_template(cfg, zbCfg, master_seed, schema, session_id, tape_id, segment_id, window_id, tlen, rs)
%MAKE_TEMPLATE Build a deterministic Zigbee replay template snippet.
