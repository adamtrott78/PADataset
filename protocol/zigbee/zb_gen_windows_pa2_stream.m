function [Xsig, meta, sched] = zb_gen_windows_pa2_stream(cfg, session_id, tape_id, segment_id, plan)
%ZB_GEN_WINDOWS_PA2_STREAM Build Zigbee PA2 windows in streaming style.
% Mirrors Wi-Fi/Bluetooth PA2:
%   - one segment-level burst schedule
%   - windows are cropped views of that schedule
%   - dense Zigbee packets inside each burst
    ...
    while t <= L_need && wi <= M
        ...
        % generate enough Zigbee packets to cover this burst, then truncate
