function bt_eval_pilot()
%BT_EVAL_PILOT Evaluate saved Bluetooth pilot windows and emit evidence pack.

    P = pa_paths();
    cfg = pa_load_cfg(fullfile(P.config, "starter.json"));
    pa_validate_cfg(cfg);

    data_root = fullfile(pa_root(), "data", "bluetooth", "digital", "pilot");
    out_root  = fullfile(pa_root(), "results", "bluetooth", "digital", "bt_eval_pilot");
    ...
    fprintf("\n=== Bluetooth cross-PA acceptance rates (columns are validators) ===\n");
    disp(T);
...
function write_typical(ev_root, cfg, typical)
    ...
    for pa = PAs
        ...
        bt_pilot_plot_evidence(pa, cfg, ex.x, ex.vr, out, []);
    end
end
...
function write_special(ev_root, cfg, special)
    ...
    if isfield(special,"PA4_as_PA3")
        ...
        bt_pilot_plot_evidence("PA4", cfg, ex.x, ex.vr_pa4, out, ex.vr_pa3);
    end

    if isfield(special,"PA4_rejected")
        ex = special.PA4_rejected;
        out = fullfile(d, sprintf("PA4_rejected_idx%03d.png", ex.idx));
        bt_pilot_plot_evidence("PA4", cfg, ex.x, ex.vr, out, []);
    end
end


function bt_pilot_plot_evidence(pa, cfg, x, vr, out_png, vr_alt)
% 3-panel figure:
%  (1) envelope + evidence mask overlay (+ repeat intervals for PA8)
%  (2) spectrogram
%  (3) centroid-bin trace
    ...
    ttl = sprintf("Bluetooth %s | valid=%d | %s", pa, vr.valid, vr.reject_reason);
    if ~isempty(vr_alt)
