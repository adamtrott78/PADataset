function [Xsig, meta, sched] = bt_gen_windows_pa4_stream(cfg, session_id, tape_id, segment_id, plan)
%BT_GEN_WINDOWS_PA4_STREAM Dense BLE PA4 windows using explicit IQ frequency offsets.
    ...
    df_grid = double(pa_get_nested(cfg, "bluetooth_base_waveform.offset_grid_hz"));
    df_grid = df_grid(:);

    assert(~isempty(df_grid), "bluetooth_base_waveform.offset_grid_hz must be nonempty.");
    ...
    for i = 1:double(M)
        ...
        % Deterministic PA4 operator RNG for this window
        ...
        ci = 1;
        pkt_count_total = 0;
        phi = 0;   % phase continuity across dwell blocks
        ...
        while cursor <= double(W)
            ...
            % Dense packet train: no intentional gaps inside the dwell
            ...
            % Make PA4 visually real: explicit frequency translation in IQ
