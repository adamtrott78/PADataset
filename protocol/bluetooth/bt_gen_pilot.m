function bt_gen_pilot(n_per_pa)
%BT_GEN_PILOT Generate and save Bluetooth pilot windows for PA2/PA3/PA4/PA8.
    ...
    btCfg = bt_make_ble_cfg(cfg); %#ok<NASGU>
    ...
    out_root = fullfile(pa_root(), "data", "bluetooth", "digital", "pilot");
    ...
    for pa = PAs
        ...
        switch pa
            case "PA2"
                [Xsig_all, meta] = bt_gen_windows_pa2_stream(cfg, session_id, tape_id, segment_id, plan);
                sched = [];

            case "PA3"
                [Xsig_all, meta] = bt_gen_windows_pa3_stream(cfg, session_id, tape_id, segment_id, plan);
                sched = [];

            case "PA4"
                [Xsig_all, meta, sched] = bt_gen_windows_pa4_stream(cfg, session_id, tape_id, segment_id, plan);

            case "PA8"
                window_ids = int64((1:n_per_pa).');
                [Xsig_all, sched] = bt_gen_windows_pa8_stream(cfg, session_id, tape_id, plan, window_ids);
