...
%   tx_stream_tape_v03("192.168.10.2", 2.437e9, 20, "TX/RX")
    ...
    % beacon frame: force COMPLEX beacon preamble (non-zero Q) so tx locks to complex
    beacon = complex(zeros(p.frameLen,1,"single"), zeros(p.frameLen,1,"single"));

    bp = single(sync.beacon_preamble(:));
    bp = bp(1:p.Lpre);

    % put energy on BOTH I and Q (same chips, 45° rotation)
    bp_c = bp .* complex(single(1/sqrt(2)), single(1/sqrt(2)));

    beacon(1:p.Lpre) = bp_c;

    % sanity: ensure transmitter sees complex on first call
...
%to_csingle = @(v) complex(single(v), zeros(size(v), "single"));