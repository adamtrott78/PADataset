...
LOG="results/buh_logs/unified_core_splitpa1_cache_v03_${TS}.log"
IMPORTANT="results/buh_logs/unified_core_splitpa1_cache_v03_${TS}_important.log"

exec > >(tee -a "$LOG") 2>&1

echo "=== UNIFIED CORE SPLIT-PA1 CACHE V03 $TS ==="
echo "REPO=$REPO"
echo "PY=$PY"
...
echo "=== PRECHECK: no active PA1 core split MATLAB workers ==="
...
"$MATLAB_BIN" -batch "cd('$REPO'); pa_relabel_bank_labels_v01({'wifi','bluetooth','zigbee'}, '$CORE_BANK', {'PA1','PA2','PA3','PA4','PA8'});" \
  2>&1 | tee "results/buh_logs/relabel_core_v03_${TS}.log"
echo ""

echo "=== BUILD PA1 CORE SPLIT PARTS ==="
PART_TASKS="results/bank_pa1_split_workers/core_pa1_part_tasks_v03_${TS}.txt"
: > "$PART_TASKS"
...
' :::: "$PART_TASKS" \
  2>&1 | tee "results/bank_pa1_split_workers/core_pa1_parts_v03_${TS}_all.log"
...
CACHE_TASKS="results/cache_workers/unified_core_cache_tasks_v03_${TS}.txt"
: > "$CACHE_TASKS"
...
' :::: "$CACHE_TASKS" \
  2>&1 | tee "results/cache_workers/unified_core_cache_v03_${TS}_all.log"
...
echo "Relabel log  : results/buh_logs/relabel_core_v03_${TS}.log"
echo "Split log    : results/bank_pa1_split_workers/core_pa1_parts_v03_${TS}_all.log"
echo "Cache log    : results/cache_workers/unified_core_cache_v03_${TS}_all.log"
