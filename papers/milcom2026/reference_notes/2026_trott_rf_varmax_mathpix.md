...
\author{
Adam Trott \\ University of Massachusetts \\ Dartmouth \\ atrott@umassd.edu
}

\author{
Henry Thompson \\ University of Massachusetts Dartmouth \\ hthompson3@umassd.edu
}
...
\caption{Table 1. Nine modulation groups selected for training}
\begin{tabular}{|l|l|}
\hline Group & Modulations \\
...
\subsection*{2.1. Modulation Grouping}
...
\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 2. Classifier architecture}
\begin{tabular}{|l|l|}
\hline Individual Branch's Channel & Fused Features \\
\hline \begin{tabular}{l}
Conv1D( $1 \rightarrow 32$ ), $\mathrm{k}=5, \mathrm{~s}=1, \mathrm{p}=2$ \\
+ BatchNorm1D(32) \\
+ LeakyReLU
\end{tabular} & \begin{tabular}{l}
Linear(1024 → 512) \\
...
\end{tabular} \\
\hline \begin{tabular}{l}
Conv1D $(32 \rightarrow 64), \mathrm{k}=3, \mathrm{~s}=2, \mathrm{p}=1$ \\
+ BatchNorm1D(64) \\
+ LeakyReLU
\end{tabular} & \begin{tabular}{l}
Linear(512 → 256) \\
...
+ Flatten → 1024 \\
(128-D feature per channel)
\end{tabular} & \begin{tabular}{l}
Linear(256 → 7) \\
+ Softmax
\end{tabular} \\
\hline 8 branches concatenated ⇒ fused feature vector of size 1024 & \\
\hline
\end{tabular}
\end{table}

\subsection*{2.2 Multi-Domain CNN Architecture}

Table 2 summarizes the architecture of our neural network. Four signal transforms each produce two channels (real and imaginary/magnitude+phase), yielding an $8 \times 1024$ tensor per frame. Eight identical 1-D convolution branches extract local features, which
are concatenated and compressed to a 256-D latent vector before the final linear classifier.

\section*{2.3. varMax Decision Logic}
...
\subsection*{3.1. Top-Two Confidence Estimation}

For each input signal x, our classifier produces a logit vector $\mathbf{z} \in \mathbf{R}^{C}$, where $C$ is the number of known classes. We apply the softmax function

$$
\begin{equation*}
S(\mathbf{z})_{i}=\frac{e^{z_{i}}}{\sum_{j=1}^{C} e^{z_{j}}}, \quad i=1, \ldots, C \tag{1}
\end{equation*}
$$

to compute the class probabilities $\mathbf{p}$. The top-two confidence score $\Delta_{2}$ is defined as the absolute difference between the two highest probabilities: $\Delta_{2}=\mid p_{(1)}- p_{(2)} \mid$. If $\Delta_{2}$ exceeds a threshold $\tau$, the prediction is accepted as a known class. Otherwise, the input is flagged for further scrutiny by subsequent tests.

\subsection*{3.2. Variance-Based Confidence Scoring}

When $\Delta_{2}<\tau$, we compute the variance of the logits before softmax activation to estimate model uncertainty. For input $\mathbf{x}$ with logits $\mathbf{z}=\left[z_{1}, \ldots, z_{C}\right]$, we calculate:

$$
\begin{equation*}
\mathrm{V}(x)=\frac{1}{N} \sum_{j=1}^{N}\left|z_{j}-\operatorname{mean}(|z|)\right|^{2} \tag{2}
\end{equation*}
$$
...
\subsection*{3.3. Energy-Based Outlier Detection}

For additional OOD verification, we calculate an energy score $E(\mathbf{x})$ for each input $\mathbf{x}$ :

$$
\begin{equation*}
E(\mathbf{x})=-\log \sum_{j=1}^{C} \exp \left(z_{j}\right) \tag{3}
\end{equation*}
$$
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/5a8a68e4-f6e9-4fc7-aee2-fd42ae90e059-04.jpg?height=966&width=1671&top_left_y=197&top_left_x=227}
\captionsetup{labelformat=empty}
\caption{Figure 1. Per-class energy and logit variance score spread with higher and lower unknown scores}
\end{figure}
...
\subsection*{3.4. Decision Cascade}

At inference time, each sample is classified using the following decision cascade:
1. If $\Delta_{2}>\tau_{\Delta}$ : classify as known.
2. Else if $\tau_{\text {var }}^{(\text {cmin })}<V(x)<\tau_{\text {var }}^{(\text {cmax })}$ : classify as known.
3. Else if $\tau_{E}^{(\text {cmin })}<E(\mathbf{x})<\tau_{E}^{(\text {cmax })}$ : classify as known.
...
\subsection*{3.5. Setting the Thresholds}
...
\subsection*{4.1. Dataset Composition}

The dataset includes 24 modulation types, each sampled across 26 signal-to-noise ratios (SNRs) from -20 dB to +30 dB in 2 dB steps with 4096 frames per modulation/SNR combination. Each frame is represented as a $2 \times 1024$ matrix with separate I and Q channels.

\subsection*{4.2. Model training}
...
$$
\begin{equation*}
\mathcal{L}=\mathcal{L}_{\mathrm{CE}}+\lambda \mathcal{L}_{\text {Center }} \tag{4}
\end{equation*}
$$

where $\lambda=.1$, and where $\mathcal{L}_{\mathrm{C}}$ is defined as:

$$
\mathcal{L}_{C}=\frac{1}{2} \sum_{i=1}^{m}\left\|\mathbf{x}_{i}-\mathbf{c}_{y_{i}}\right\|_{2}^{2}
$$

and $\mathcal{L}_{\mathrm{CE}}$ is defined as:

$$
\mathcal{L}_{\mathrm{CE}}=-\sum_{i=1}^{m} \log p_{y_{i}}
$$


\subsection*{4.3. Group Partitioning}

The modulation types have been split into nine semantically and structurally coherent groups, as shown in Table 1, and we selected one modulation type from each group to train the model on, with a range of SNR values from +6 dB to +30 dB . This ensures that each group is represented in the closed set of our data.

\subsection*{4.4. Evaluation Strategy}

The model's effectiveness is calculated by having one modulation type be the unknown group, cycling through all nine modulation types, and calculating the $F 1$-scores for each fold. Each F1-score is calculated based on its accuracy in differentiating known and unknown data to focus on its OSR performance and not the accuracy of the classifier. Each fold is then ran 30 times, and the average performance of each fold is recorded in the results
...
\subsection*{5.1. Model Performance}

\begin{table}
\captionsetup{labelformat=empty}
\caption{Table 3. Leave-one-group-out OSR performance}
\begin{tabular}{|l|l|l|l|}
\hline Unknown Group & F1-Score & Unknown Acc. (\%) & Known Acc. (\%) \\
\hline 4ASK & 0.976 & 83 & 98 \\
\hline AM-DSB-WC & 0.895 & 83 & 92 \\
\hline AM-SSB-WC & 0.991 & 99 & 98 \\
\hline 16APSK & 0.822 & 85 & 80 \\
\hline FM & 0.931 & 84 & 94 \\
\hline OQPSK & 0.877 & 76 & 97 \\
\hline GMSK & 0.763 & 69 & 77 \\
\hline 32PSK & 0.737 & 75 & 71 \\
\hline 64QAM & 0.72 & 69 & 73 \\
\hline
\end{tabular}
\end{table}

Table 3 summarizes fold-wise results. Showing the average $F 1$-score, the average unknown accuracy (percent of OOD flagged as unknown) and the average known accuracy (percent of known data not flagged). A notable result of this experiment is the wide variety of $F 1$-scores when the open set is rotated. Our highest scoring fold was with AM-SSB-WC, with a 0.992 F1-score, and unknown and known accuracies of $99 \%$ and $98 \%$, respectively.

It can be concluded that owing to the distinct nature of the AM-SSB-WC waveform, the variance in the resulting logits is extremely high, resulting in very accurate unknown detection. However, when the modulation type AM-DSB-WC is the unknown set, the $F 1$-score is 895 , with unknown accuracy $83 \%$ and known accuracy $92 \%$. The only difference between these two modulation types is that AM-DSB-WC has both upper and lower sidebands transmitted, whereas AM-SSB-WC has only one. Because AM-SSB-WC only has one sideband, its constellation diagram has IQ values concentrated in one hemisphere, whereas essentially every other modulation type, including AM-DSB-WC, has IQ values situated symmetrically around the origin. This unique representation could
...
Figure 2 shows the model's average unknown $F 1$-score per-fold, as well as the value of the F1-scores one standard deviation higher and lower than the mean. Our model provides very consistent results, and the standard deviation appears to decrease as the model performance increases.

\subsection*{5.2. Baseline Performance}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/5a8a68e4-f6e9-4fc7-aee2-fd42ae90e059-06.jpg?height=401&width=823&top_left_y=212&top_left_x=1100}
\captionsetup{labelformat=empty}
\caption{Figure 2. Average Unknown $F_{1}$-Score and Standard Deviation Per-Fold}
\end{figure}
...
Figure 3 shows the average unknown $F 1$-scores across all folds of our model alongside our four baseline models. Classic Varmax, energy, and variance perform very poorly due to the fact that these models only include a single threshold, which when provided with
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/5a8a68e4-f6e9-4fc7-aee2-fd42ae90e059-07.jpg?height=525&width=824&top_left_y=586&top_left_x=244}
\captionsetup{labelformat=empty}
\caption{Figure 3. Average Unknown $F_{1}$-Score of proposed model vs. baseline models}
\end{figure}

\subsection*{5.3. Model Performance across SNRs}

This model was designed to perform under an extensive range of operational conditions. Therefore, the model was trained using a wide variety of SNR values. Although our results are fairly positive, it is clear that when the SNR of the sample is lower than the range in our training data, the effectiveness of the model decreases significantly. Figure 5 shows the model's F1-score when tested at individual SNR values ranging from -30 to +20 . Our model was trained on SNR values ranging from +6 to +30 . For most of the folds, SNR values of +6 and above did not have any significant impact on performance. However, when testing SNR values under +6 , the model showed a substantial decrease in performance. Additionally, Figure 5i shows OQPSK with $F 1$-scores of approximately . 5 from SNRs of +16 to $+30, F 1$-scores ranging from .59 to .79 when tested on SNR values of -6 to +12 . We believe that this outlier is due to the fact that the model with OQPSK as the unknown set is in a unique position to make this determination. At high SNR values, the model confidently predicts unknowns to be 32PSK or 16APSK, leading to low unknown $F 1$-scores. At lower SNR values, this certainty of both classes turns into uncertainty between the two, leading to higher energy and lower variance scores, and therefore increased unknown detection. Overall, the performance of our
...
\subsection*{5.4. Evaluating Model Bias}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/5a8a68e4-f6e9-4fc7-aee2-fd42ae90e059-08.jpg?height=1456&width=1639&top_left_y=216&top_left_x=244}
\captionsetup{labelformat=empty}
\caption{Figure 4. Anomaly detection latent space representation: k-means vs varMax}
\end{figure}
...
\begin{figure}
\includegraphics[alt={},max width=\textwidth]{https://cdn.mathpix.com/cropped/5a8a68e4-f6e9-4fc7-aee2-fd42ae90e059-09.jpg?height=1357&width=1675&top_left_y=197&top_left_x=225}
\captionsetup{labelformat=empty}
\caption{Figure 5. evaluating $F_{1}$-scores per SNR value with each modulation type as unknown}
\end{figure}

\section*{7. Acknowledgment}

This material is in part based upon work supported by the National Science Foundation under Grant Number 2234915. Any opinions, findings, conclusions, or recommendations expressed in this material are those of the authors and do not necessarily reflect the views of the National Science Foundation.

\section*{References}

Baye, G., Silva, P., Broggi, A., Bastian, N., Fiondella, L., \& Kul, G. (2024). Varmax: Towards confidence-based zero-day attack recognition. 2024 IEEE Military Communications Conference (MILCOM), 863-868. https://doi. org/10.1109/MILCOM61039.2024.10773707

Berenbeim, A. M., Bierbrauer, D. A., Cruickshank, I. J., Thomson, R. H., \& Bastian, N. D. (2023, October). Applications of certainty scoring for machine learning classification in multi-modal contexts [Preprint]. https://doi.org/10.36227/ techrxiv.24257146.v1
Broggi, A., Baye, G., Silva, P., Costagliola, N., Bastian, N., Fiondella, L., \& Kul, G. (2025). Varmax: Uncertainty and novelty management in deep neural networks. Proceedings of the 58th Hawaii International Conference on System Sciences (HICSS), 7514-7523. https://doi.org/ 10.24251/HICSS.2025.898

Ge, Z., Demyanov, S., Chen, Z., \& Garnavi, R. (2017). Generative OpenMax for multi-class open set classification. Proceedings of the British Machine Vision Conference (BMVC), 42.1-42.12. https://doi.org/10.5244/C.31.42

O'Shea, T., \& West, N. (2018). Radioml 2016.10a and 2018.01a [data set]. https://www.kaggle.com/ datasets/pinxau1000/radioml2018
...
Shi, Y., Davaslioglu, K., Sagduyu, Y. E., Headley, W. C., Fowler, M., \& Green, G. (2019). Deep learning for RF signal classification in unknown and dynamic spectrum environments. 2019 IEEE International Symposium on Dynamic Spectrum Access Networks (DySPAN), 1-10. https://doi.org/10.1109/DySPAN.2019. 8935684
Wang, Y., Li, B., Che, T., Zhou, K., Liu, Z., \& Li, D. (2021). Energy-based open-world uncertainty modeling for confidence calibration. Proceedings of the IEEE/CVF International Conference on Computer Vision (ICCV), 9282-9291. https://doi.org/10.1109/ ICCV48922.2021.00917
Wei, X., Saha, D., \& Quach, A. (2024). Exploiting multi-domain features for detection of unclassified electromagnetic signals. Proceedings of the 2024 IEEE Military Communications Conference (MILCOM), 1-6. https://doi.org/10.1109/MILCOM61039.2024. 10773991