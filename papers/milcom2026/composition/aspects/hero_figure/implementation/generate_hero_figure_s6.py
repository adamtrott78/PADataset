#!/usr/bin/env python3
"""
Generate Hero Figure Candidate S6.

S6 is a readability and arrow-geometry refinement of S5:
  - formal panel names;
  - cleaner font stack;
  - shorter labels;
...
def build_svg() -> str:
    ...
    # Variables are on the same line as names for readability.
    metric_x, metric_w, metric_h = 835, 170, 56
    metric_ys = [132, 215, 298, 381]
    metrics = [
        "ŷ  predicted PA",
        "z  logits",
        "p  softmax",
    ...
    for label, y in zip(metrics, metric_ys):
        s.append(rect(metric_x, y, metric_w, metric_h, COLORS["box"], sw=1.7, rx=8))
        s.append(text(metric_x + metric_w / 2, y + metric_h * 0.58, label, "metric"))
    ...
    stages = [
        ...
        (stage2, "2", "Guard evidence", ["P₁, P₁ − P₂, H", "V, E, r(x)"]),
        (stage3, "3", "Known-budget threshold", ["r(x) ≥ τβ", "β = 0.05"]),
    ]
    ...
    for (x, y, w, h), num, title, detail in stages:
        ...
        s.append(lines(x + w / 2 + 18, y + 64, detail, "detail", line_h=17))
...
def main() -> None:
    ...
    parser.add_argument("--name", default="hero_dqnguard_pipeline_s6_horizontal")
