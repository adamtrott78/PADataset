#!/usr/bin/env python3
"""
Generate Hero Figure Candidate S9.

S9 is a direct inline-SVG metric typography repair of S8:
  - formal panel names;
  - cleaner font stack;
  - shorter labels;
...
def build_svg() -> str:
    ...
    for (name, var), y in zip(metrics, metric_ys):
        ...
        s.append(
            f'<text x="{metric_x + metric_w / 2}" y="{y + 30}" text-anchor="middle" '
            f'style="font-family: Liberation Sans, Arial, Helvetica, sans-serif; '
            f'font-size: 24px; font-weight: 700; fill: #18212b;">{esc(name)}</text>\n'
        )
        s.append(
            f'<text x="{metric_x + metric_w / 2}" y="{y + 62}" text-anchor="middle" '
            f'style="font-family: Liberation Serif, Times New Roman, Times, serif; '
            f'font-size: 30px; font-style: italic; fill: #18212b;">{esc(var)}</text>\n'
        )
...
def main() -> None:
    ...
    parser.add_argument("--name", default="hero_dqnguard_pipeline_s9_horizontal")
