#!/usr/bin/env python3
"""
Generate Hero Figure Candidate S8.

S8 is a metric typography and math-styling repair of S7:
  - formal panel names;
  - cleaner font stack;
  - shorter labels;
...
def build_svg() -> str:
    ...
    metric_x, metric_w, metric_h = 825, 185, 66
    metric_ys = [122, 209, 296, 383]
    metrics = [
        ("predicted PA", "ŷ"),
        ("logits", "z"),
        ("softmax", "p"),
    ...
    for (name, var), y in zip(metrics, metric_ys):
        ...
        s.append(text(metric_x + metric_w / 2, y + 25, name, "metriclabel"))
        s.append(text(metric_x + metric_w / 2, y + 53, var, "metricvar"))
    ...
    for (x, y, w, h), num, title, detail in stages:
        ...
        s.append(lines(x + w / 2 + 18, y + 64, detail, "detail", line_h=16))
...
def main() -> None:
    ...
    parser.add_argument("--name", default="hero_dqnguard_pipeline_s8_horizontal")
