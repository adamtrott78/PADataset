#!/usr/bin/env python3
"""
Generate Hero Figure Candidate S5.

S5 is a typography/layout refinement of S4:
  - formal panel names;
  - cleaner font stack;
  - shorter labels;
...
def header() -> str:
    return (
        ...
        '<style>\n'
        '  text { font-family: "TeX Gyre Heros", "Nimbus Sans", "DejaVu Sans", Helvetica, Arial, sans-serif; fill: #18212b; }\n'
        '  .paneltitle { font-size: 25px; font-weight: 700; letter-spacing: 0.1px; }\n'
        '  .subtitle { font-size: 17px; fill: #344054; }\n'
        '  .label { font-size: 21px; font-weight: 500; }\n'
        '  .detail { font-size: 17px; fill: #2f3742; }\n'
        '  .small { font-size: 15px; fill: #384252; }\n'
        '  .tiny { font-size: 13.5px; fill: #384252; }\n'
        '  .num { font-size: 22px; font-weight: 700; }\n'
        '  .math { font-family: "Latin Modern Math", "STIX Two Math", "Times New Roman", Times, serif; font-size: 16px; font-style: italic; fill: #18212b; }\n'
        '  .mathtiny { font-family: "Latin Modern Math", "STIX Two Math", "Times New Roman", Times, serif; font-size: 13.5px; font-style: italic; fill: #18212b; }\n'
        '</style>\n'
    )
...
def build_svg() -> str:
    ...
    for label, y in zip(transforms, trans_ys):
        ...
        s.append(arrow([(win[0] + win[2], win[1] + win[3] / 2), (trans_x - 7, y + trans_h / 2)], sw=1.65, head=6.5))
    ...
    for y in trans_ys:
        s.append(arrow([(trans_x + trans_w, y + trans_h / 2), (fusion[0] - 7, fusion[1] + fusion[3] / 2)], sw=1.65, head=6.5))

    s.append(arrow([(fusion[0] + fusion[2], fusion[1] + fusion[3] / 2), (model[0] - 7, model[1] + model[3] / 2)], sw=1.8, head=7))

    # Metrics ordered for clean routing: yhat, z, p, h.
    metric_x, metric_w, metric_h = 850, 140, 51
    metric_ys = [137, 215, 293, 371]
    metrics = [
        ("predicted PA", ("ŷ", "mathtiny")),
        ("logits", ("z", "mathtiny")),
        ("softmax", ("p", "mathtiny")),
        ("feature", ("h", "mathtiny")),
    ]

    for (title, detail), y in zip(metrics, metric_ys):
        s.append(box(metric_x, y, metric_w, metric_h, COLORS["box"], title, [detail], title_cls="tiny", detail_cls="mathtiny", rx=8, sw=1.7))
        s.append(arrow([(model[0] + model[2], model[1] + model[3] / 2), (metric_x - 7, y + metric_h / 2)], sw=1.55, head=6.2))
    ...
    stages = [
        (stage1, "1", "Predicted-class calibration", ["select bands for", ("ŷ", "mathtiny")]),
    ...
    for (x, y, w, h), num, title, detail in stages:
        ...
        s.append(lines(x + w / 2 + 18, y + 64, detail, "small", line_h=16))

    # Subtle internal cascade arrows.
    mid_x = gx + gw / 2
    s.append(arrow([(mid_x, stage1[1] + stage1[3] + 5), (mid_x, stage2[1] - 7)], color=COLORS["guard_stroke"], sw=1.4, head=6.2))
    s.append(arrow([(mid_x, stage2[1] + stage2[3] + 5), (mid_x, stage3[1] - 7)], color=COLORS["guard_stroke"], sw=1.4, head=6.2))
    ...
    s.append(arrow([(metric_x + metric_w, yhat_y), (stage1[0] - 7, stage1[1] + stage1[3] / 2)], sw=1.8, head=7))

    # z, p, h -> stage 2. Use separate landing heights to avoid one thick bundle.
    stage2_targets = [stage2[1] + 31, stage2[1] + stage2[3] / 2, stage2[1] + stage2[3] - 31]
    for src_y, tgt_y in zip([z_y, p_y, h_y], stage2_targets):
        s.append(arrow([(metric_x + metric_w, src_y), (stage2[0] - 7, tgt_y)], sw=1.65, head=6.5))
    ...
    s.append(box(*attack, COLORS["attack_fill"], "ATT&CK/EW", ["precursor hypotheses"], stroke=COLORS["attack_stroke"], title_cls="detail", detail_cls="tiny", rx=9, sw=1.8))
    s.append(box(*llm, COLORS["llm_fill"], "LLM-assisted", ["label-making"], stroke=COLORS["llm_stroke"], title_cls="detail", detail_cls="tiny", rx=9, sw=1.8))
    s.append(box(*qrcwos, COLORS["qrcwos_fill"], "QR-CWoS", ["response planning"], stroke=COLORS["qrcwos_stroke"], title_cls="detail", detail_cls="tiny", rx=9, sw=1.8))

    # DQNGuard decision branch arrows.
    s.append(arrow([(gx + gw, stage2[1] - 20), (known[0] - 7, known[1] + known[3] / 2)], color=COLORS["known_stroke"], sw=1.9, head=7.5))
    s.append(arrow([(gx + gw, stage3[1] + 15), (unknown[0] - 7, unknown[1] + unknown[3] / 2)], color=COLORS["unknown_stroke"], sw=1.9, head=7.5))

    # Consumer arrows land exactly on left boundaries.
    s.append(arrow([(known[0] + known[2], known[1] + known[3] / 2), (attack[0] - 7, attack[1] + attack[3] / 2)], color=COLORS["known_stroke"], sw=1.65, head=6.5))
    s.append(arrow([(unknown[0] + unknown[2], unknown[1] + unknown[3] / 2), (llm[0] - 7, llm[1] + llm[3] / 2)], color=COLORS["unknown_stroke"], sw=1.65, head=6.5))
    s.append(arrow([(unknown[0] + unknown[2], unknown[1] + unknown[3] / 2), (qrcwos[0] - 7, qrcwos[1] + qrcwos[3] / 2)], color=COLORS["unknown_stroke"], sw=1.65, head=6.5))
...
def main() -> None:
    ...
    parser.add_argument("--name", default="hero_dqnguard_pipeline_s5_horizontal")
