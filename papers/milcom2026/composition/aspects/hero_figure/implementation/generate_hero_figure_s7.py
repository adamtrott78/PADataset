#!/usr/bin/env python3
"""
Generate Hero Figure Candidate S7.

S7 is a metric-readability and arrow-endpoint repair of S6:
  - formal panel names;
  - cleaner font stack;
  - shorter labels;
...
def build_svg() -> str:
    ...
    # Variables are below the names, with larger text for readability.
    metric_x, metric_w, metric_h = 835, 170, 62
    metric_ys = [126, 211, 296, 381]
    metrics = [
        ("predicted PA", "ŷ"),
        ("logits", "z"),
        ("softmax", "p"),
    ...
    for (name, var), y in zip(metrics, metric_ys):
        ...
        s.append(text(metric_x + metric_w / 2, y + 24, name, "metriclabel"))
        s.append(text(metric_x + metric_w / 2, y + 49, var, "metricvar"))
...
def main() -> None:
    ...
    parser.add_argument("--name", default="hero_dqnguard_pipeline_s7_horizontal")
