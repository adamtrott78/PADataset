...
%   tx_stream_tape(protocol, ip, fc_hz, gain_db, tx_ant, tape_file_override)
    ...
    try
        % --------- BEACON MODE ---------
        beacon_start_t = tic;
        auto_go_after_s = 10.0;    % SSH-friendly fallback
        settle_pause_s  = 1.0;   % short quiet pause before main tape
        ...
        while ~st.go && ~st.quit
            ...
            if toc(beacon_start_t) >= auto_go_after_s
                st.go = true;
                fprintf("TX auto-trigger after %.1f s\n", auto_go_after_s);
            end
        ...
        fprintf("TX: SPACE pressed -> preparing %s tx_tape (%d samples)\n", ...
            protocol_s, numel(tx_tape));

        % --------- SETTLE PAUSE ---------
        fprintf("TX settle pause %.2f s before main tape...\n", settle_pause_s);
        pause(settle_pause_s);

        fprintf("TX: streaming %s tx_tape now.\n", protocol_s);
